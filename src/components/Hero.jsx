import { Card, Row, Col, Carousel, Button, Modal, Form, Alert, Spinner, } from "react-bootstrap";
import "../css/hero.css";
...
const Hero = () => {
  ...
  const handleSave = async () => {
    ...
    const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2OGI1YTFkOTE2MjdjNjAwMTVmOGM1NmMiLCJpYXQiOjE3NTY3MzM5MTMsImV4cCI6MTc1Nzk0MzUxM30.SOLseepU4Ysb0KnFQYR3yWP1jikhGc89-HCynCKAhuY";
    if (!token) {
      setAlert({ type: "danger", message: "Token mancante. Effettua il login." });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (contentType && contentType.includes("text/html")) {
          const errorMessage = "Errore di rete o token scaduto. Verifica il login.";
          setAlert({ type: "danger", message: errorMessage });
          return;
        }

        const errorData = await response.json();
        const errorMessage = errorData.message || "Errore durante l'aggiornamento.";
        setAlert({ type: "danger", message: errorMessage });
        return;
      } else {
  ...
  return (
    <Card className="border border-1 rounded-4 m-4 position-relative shadow">
      <Card.Img
        variant="top" className="hero-img" src="https://png.pngtree.com/background/20230408/original/pngtree-mountain-view-in-the-morning-picture-image_2336856.jpg"
      />
      ...
      {!id && (
        <div className="picture-photocamera" onClick={handleCameraClick} style={{ cursor: !id ? "pointer" : "default" }}>
          <BiSolidCamera />
        </div>
      )}
      ...
      <Card.Body className="container-fluid mt-5">
        <Row>
          ...
          <Col xs={6} lg={6} className="d-flex gap-2 d-none d-md-block justify-content-end">
            <img
              src="https://seeklogo.com/images/U/universita-degli-studi-di-padova-logo-D71B2107E1-seeklogo.com.png"
              alt="Logo uni" className="logo-uni"
            />
            ...
            {!id && (
              <BiPencil className="position-absolute top-0 end-0 me-2" style={{ cursor: "pointer", fontSize: "1.5rem", marginTop: "18%" }} onClick={handleShowUpgradeModal} />
            )}
          </Col>
        </Row>
        {!id && (
          <div className="mt-3 d-flex flex-wrap gap-2">
            <div className="badge-info badge-active" title="Not available">Disponibile per</div>
            <div className="badge-info myBlue" title="Not available">Aggiungi sezione del profilo</div>
            <div className="badge-info myBlue" title="Not available">Migliora profilo</div>
            <div className="badge-info border-black" title="Not available">Risorse</div>
          </div>
        )}
        {!id && (
          <Carousel data-bs-theme="dark" className="d-sm-none">
            <Carousel.Item interval={120000}>
              <img
                className="d-block w-100 carousel-img-info" alt="First slide" src="https://t3.ftcdn.net/jpg/03/53/11/00/360_F_353110097_nbpmfn9iHlxef4EDIhXB1tdTD0lcWhG9.jpg"
              />
            ...
            <Carousel.Item interval={120000}>
              <img
                className="d-block w-100 carousel-img-info" alt="Second slide" src="https://t3.ftcdn.net/jpg/03/53/11/00/360_F_353110097_nbpmfn9iHlxef4EDIhXB1tdTD0lcWhG9.jpg"
              />
        ...
        <Modal.Body>
          ...
          {imageFile && (
            <div className="text-center mb-3">
              <img src={URL.createObjectURL(imageFile)} alt="Preview" className="img-fluid rounded" style={{ maxHeight: "200px", maxWidth: "100%" }} />
            </div>
          )}
        ...
        <Modal.Body>
          ...
          <Form>
            <Form.Group className="mb-3">
              ...
              <Form.Control type="text" placeholder="Inserisci il tuo nome" value={editName} onChange={(e) => setEditName(e.target.value)} />
            </Form.Group>

            <Form.Group className="mb-3">
              <Form.Label>Cognome*</Form.Label>
              <Form.Control type="text" placeholder="Inserisci il tuo cognome" value={editSurname} onChange={(e) => setEditSurname(e.target.value)} />
            </Form.Group>

            <Form.Group className="mb-3">
              <Form.Label>Sommario*</Form.Label>
              <Form.Control type="text" placeholder="Inserisci il tuo sommario" value={editSummary} onChange={(e) => setEditSummary(e.target.value)} />
            </Form.Group>

            <Form.Group className="mb-3">
              <Form.Label>Città*</Form.Label>
              <Form.Control type="text" placeholder="Inserisci la tua città" value={editCity} onChange={(e) => setEditCity(e.target.value)} />
            </Form.Group>

            <Form.Group className="mb-3">
              <Form.Label>Formazione*</Form.Label>
              <Form.Control type="text" placeholder="Inserisci la tua formazione" value={editCity} onChange={(e) => setEditCity(e.target.value)} />
            </Form.Group>
          </Form>
        </Modal.Body>
